...
import { addMonthsKeepingDay } from '../../utils/dateUtils';
...
function FieldRow({ label, error, children, unit }: { label: string; error?: string; children: React.ReactNode; unit?: string }) {
    return (
        <div className="space-y-1">
            <div className="flex items-center justify-between">
                <label className="text-xs font-medium text-current opacity-70">{label}</label>
...
function ProfileSheetContent({ stats, goal, language, waterTarget, weightLog, activeTab, setActiveTab, setStats, setGoal, setLanguage, setWaterTarget, clearAll, onClose }: ProfileSheetContentProps) {
    ...
    const handleReset = () => {
        if (window.confirm(copy.resetConfirm)) { clearAll(); onClose(); }
    };
            ...
            <aside className={`relative flex h-full w-full max-w-lg flex-col shadow-2xl ${previewTheme.pageBg} animate-in slide-in-from-right duration-300`}>
                ...
                <div className={`flex shrink-0 items-center justify-between border-b px-5 py-4 ${previewTheme.cardBorder} ${previewTheme.cardBg}`}>
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            title={!canSave ? Object.values(validationErrors).join('\n') : ''}
                        >
                            {copy.save}
                        </button>
                        <button type="button" onClick={onClose} className={`grid h-9 w-9 place-items-center rounded-full border transition ${previewTheme.cardBorder} ${previewTheme.mutedSurface} hover:opacity-80`}>
                            <X className="h-4 w-4" />
                        </button>
                    </div>
                </div>
                ...
                <div className={`flex shrink-0 border-b ${previewTheme.cardBorder} ${previewTheme.cardBg}`}>
                    {(['profile', 'weight'] as const).map((tab) => (
                        <button
                            key={tab}
                            className={`flex-1 py-3 text-sm font-semibold transition ${activeTab === tab ? `${previewTheme.accent} border-b-2 border-current` : previewTheme.subtitle}`}
                        ...
                        <>
                            ...
                            >
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-3 gap-3">
                                        <FieldRow label={copy.weight} unit="kg" error={validationErrors.weight}>
                                            <input type="number" min="1" value={draft.weight} onChange={(e) => update('weight', e.target.value)} className={inputBase} />
                                        </FieldRow>
                                        <FieldRow label={copy.height} unit="cm" error={validationErrors.height}>
                                            <input type="number" min="1" value={draft.height} onChange={(e) => update('height', e.target.value)} className={inputBase} />
                                        </FieldRow>
                                        <FieldRow label={copy.age} error={validationErrors.age}>
                                            <input type="number" min="1" value={draft.age} onChange={(e) => update('age', e.target.value)} className={inputBase} />
                                        </FieldRow>
                                    </div>
                            ...
                            >
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <FieldRow label={copy.targetWeight} unit="kg" error={validationErrors.targetWeight}>
                                            <input type="number" min="1" value={draft.targetWeight} onChange={(e) => update('targetWeight', e.target.value)} className={inputBase} />
                                        </FieldRow>
                                        <FieldRow label={copy.duration}>
                                            <select
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <FieldRow label={copy.sportDays} unit={copy.dayUnit} error={validationErrors.weeklySportQuota}>
                                            <input type="number" min="0" max="7" value={draft.weeklySportQuota} onChange={(e) => update('weeklySportQuota', e.target.value)} className={inputBase} />
                                        </FieldRow>
                                        <FieldRow label={copy.dailyWater} unit={copy.waterUnit} error={validationErrors.waterTarget}>
                                            <input type="number" min="100" step="100" value={draft.waterTarget} onChange={(e) => update('waterTarget', e.target.value)} className={inputBase} />
                                        </FieldRow>
                                    </div>
                                </div>
                            </AccordionSection>
                                        ...
                                        <div className={`rounded-xl border p-3 ${previewTheme.cardBorder} ${previewTheme.mutedSurface}`}>
                                            ...
                                            {draft.cycleEnabled && (
                                                <div className="mt-3 space-y-3">
                                                    <FieldRow label={copy.lastPeriodDate} error={validationErrors.lastPeriodDate}>
                                                        <input
                                                            type="date"
                                                    ...
                                                    <div className="grid grid-cols-2 gap-3">
                                                        <FieldRow label={copy.cycleLength} unit={draft.language === 'tr' ? 'gün' : 'days'} error={validationErrors.cycleLength}>
                                                            <input
                                                                type="number"
                                                                min="20"
                                                                max="40"
                                                        ...
                                                        <FieldRow label={copy.periodLength} unit={draft.language === 'tr' ? 'gün' : 'days'} error={validationErrors.periodLength}>
                                                            <input
                                                                type="number"
                                                                min="1"
                                                                max="15"
                        ...
                        <div className="space-y-5">
                            <div className="flex items-center justify-end gap-2">
                                {(['month', 'all'] as const).map((r) => (
                                    <button
                                        key={r}
                                        className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition ${previewTheme.cardBorder} ${chartTimeRange === r ? previewTheme.accentBtn : `${previewTheme.mutedSurface} ${previewTheme.title}`}`}
